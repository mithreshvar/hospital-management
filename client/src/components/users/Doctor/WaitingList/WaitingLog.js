...
export default function LogField({ currentWaitingList }) {
    const { dispatch } = useWaitingListsContext();
    const profileRoute = '/patients/' + currentWaitingList.rollno;
    ...
    return (
        <div className="z-0 bg-white h-[80px] w-full  flex items-center justify-between text-[25px] text-[#1a1a61db] font-Fredoka font-medium rounded-[10px] self-center hover:border-[3px] hover:border-[#e1a3ff] mb-[20px] " >
            <Link to={profileRoute} className="w-full p-[25px]">
            ...
            <div className="flex gap-x-[15px] p-[25px] ">
                <Link to='/hello' className="w-[40px] h-[40px] text-[18px] bg-[#7ed957] text-white flex items-center justify-center rounded-[100px]  " ><ImCheckmark /></Link>
